...
std::tuple<float,float> MinAndMax(float x,float y,float z)
{
    return {fmin(fmin(x,y),z),fmax(fmax(x,y),z)};
}
...
void rst::rasterizer::rasterize_triangle(const Triangle& t, const std::array<Eigen::Vector3f, 3>& view_pos) 
{
    ...
    auto v = t.toVector4();
    auto [downMost,upMost]=MinAndMax(v[0].y(),v[1].y(),v[2].y());
    auto [leftMost,rightMost]=MinAndMax(v[0].x(),v[1].x(),v[2].x());
    
    for (int y = downMost; y < upMost; y++)
    {
        for (int x = leftMost; x < rightMost; x++)
        {
            if (insideTriangle(x,y,t.v))
            {
                ...
                Vector3f interpolated_normal=(alpha*t.normal[0].normalized()+beta*t.normal[1].normalized()+gamma*t.normal[2].normalized());
